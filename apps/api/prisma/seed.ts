...
import { assertSafeToSeed } from '../src/bootstrap/seed-guard';
...
async function main() {
  // There was a second, inline guard here that wanted ALLOW_PRODUCTION_SEED=true
  // while assertSafeToSeed wanted 'yes-i-mean-it'. Each rejected the value the
  // other required, so the documented escape hatch could not be opened at all.
  assertSafeToSeed();
